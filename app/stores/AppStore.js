...
import { sendStateChange } from "../utils/appWinIPC";
import moment from "moment";
import i18n from "../utils/i18n";

import _debug from "debug";
const debug = _debug("app:stores/AppStore.jsx");
const windowListAsJson = window.require("./utils/native-sgrab-helper")
  .windowListAsJson;
...
function filterList(list) {
  ...
  return _.filter(windowList, it => {
    return (
      it.owner !== "Teasy" &&
      it.name !== "" &&
      it.owner !== "Electron" &&
      it.name.indexOf("Electron Helper") === -1
    );
  });
}
...
class AppStore {
  ...
  setBreakTime({ minutesFromNow }) {
    this.updateLiveViewTime({
      breakTime: moment().add(minutesFromNow, "minute").format("HH:mm")
    });
  }
